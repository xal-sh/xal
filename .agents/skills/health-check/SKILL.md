...
- Provider load: join request lifecycle and shape records by `request`, split `turn` from `compaction`, and compare model, thinking, attempt, outcome, usage, numeric request shape, first-event latency, and total duration. Report total provider input before cache credit separately from `totalInputTokens - cacheReadInputTokens`, which is observed uncached prefill. A jump to roughly the whole context is an observed prompt-cache miss, but cache attribution is best-effort and must not be treated as reproducible without repeated evidence.
