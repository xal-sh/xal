...
Automatic context compaction starts at 90% of the model's context window by default. A provider may advertise a lower explicit auto-compaction limit; higher limits are clamped to the 90% ceiling. The full request is checked again after compaction, and Xal never samples a normal request at or beyond the model's hard context window.
