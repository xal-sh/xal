...
Profiler records never contain prompt, response, summary, tool-output, schema, path, working-directory, argument, call-ID, real session-ID, provider-profile, or credential content. The profiler logs a redacted error and stops recording if its writer fails; a profiler failure does not alter or retry the provider request. Use `bun benchmark:context -- --fixture scripts/fixtures/context-efficiency-v1.json --policy legacy` to replay the committed content-free numeric baseline. Regenerate a fixture only from explicit local inputs with `bun benchmark:context -- --generate --profiler PROFILE_DIRECTORY --sessions SESSION_DIRECTORY --context-window TOKENS --output FIXTURE_JSON`; generation derives all workload events and the legacy baseline from matching profiler and session observations, fails when they cannot be matched uniquely, and never copies a template. Live results retain anonymous provider/model labels, a connection/model/thinking configuration fingerprint, and a per-scenario workload fingerprint so paired comparisons reject changed configuration while release runs may include additional scenarios. Paired release comparisons gate the sum of provider-reported `totalInputTokens` without cache credit because provider cache attribution is best-effort. They retain cache-adjusted input as diagnostic evidence and enforce latency and continuation per workload.

## Semantic code search

Enable [Jev code search](/docs/configs#jev-code-search) to give the model a `code_search` tool for natural-language questions such as “Where does the harness decide whether tool calls can run concurrently?” It retrieves local candidate passages, ranks them with TypeSafe's `jev-latest`, and returns source excerpts with file paths and line numbers. No summary is generated. Each complete source file is redacted using the current known-secret snapshot before it is split into passages, preserving newline counts so source line positions remain accurate. Both Jev and the returned excerpts receive this protected text, including when a multiline secret crosses a passage boundary.

Arguments:

| Argument | Meaning                                                                                      |
| -------- | -------------------------------------------------------------------------------------------- |
| `query`  | Required question, at most 2,000 UTF-8 bytes. Include likely technical terms when known.     |
| `path`   | Optional file or directory inside the session working directory. Defaults to that directory. |
| `glob`   | Optional file filter, for example `*.ts` or `src/**`.                                        |
| `limit`  | Maximum returned excerpts, 1 to 10; defaults to 5.                                           |

The native retrieval pass splits identifier words, compares query terms against paths and content, and keeps up to 40 passages. Each passage contains at most 40 lines and 1,800 UTF-8 bytes, with overlap to preserve nearby context. This is lexical retrieval followed by semantic reranking, not an embedding index. Jev cannot recover passages that retrieval did not select. Results are explicitly a shortlist, never proof that no other implementation exists. Use `grep` for exact matches, `read` for surrounding context, and LSP for known symbols.

Each call reads current disk contents without a persistent index. Search honors ignore files even for explicit targets, stays inside the working directory, and does not follow symlinks. Binary or invalid UTF-8 files and files over 512 KiB are skipped. Credential-like paths are excluded: `.env` and `.env.*`, `credentials` and `credentials.*`, `secrets` and `secrets.*`, `id_rsa*`, `id_ed25519*`, `.git`, `.ssh`, `.aws`, `.gnupg`, `.kube`, and key/certificate extensions `pem`, `key`, `p12`, `pfx`, `crt`, `cer`, `der`, and `keystore`. These exclusions can also omit source files with those names. Lines over 1,800 bytes are skipped rather than presented as complete code.

Local retrieval checks a three-second deadline and caps enumeration at 20,000 files and scanning at approximately 32 MiB. Results report scanned files, skipped files and lines, matching passages, and incomplete scans. Narrow `path` or `glob` when coverage is limited. Filesystem errors fail the tool instead of being reported as no matches.

Reranking asks one Noul relevance question per candidate in one decision-service evaluation. The serialized, redacted request is capped at 90,000 bytes by dropping the lowest-ranked local candidates as necessary. This is a conservative size estimate, not an exact tokenizer bound. The TypeSafe attempt, including transport retries, has a three-second deadline. API errors, invalid scores, a disconnected profile, or a timeout return local rankings with an explicit fallback notice. User cancellation stops without fallback; an empty local shortlist does not call TypeSafe. Overlapping results are removed after ranking. Successful responses contribute TypeSafe usage under the `code_search` phase.

This tool is declared read-only and supports shared execution. Enabling code search does not enable Jev compaction, perform background searches, or replace existing tools.
