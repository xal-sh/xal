...
- The navigator at the bottom lists every process, task agent, and schedule: running rows first, then finished rows (newest first). Schedule rows show their remaining time. Finished rows, including failed, stopped, and timed-out work, are automatically dismissed when the primary session accepts your next message. Queued, running, and stopping work stays visible. Returning to idle or receiving an automatic background result does not dismiss rows; you can still dismiss them manually, and the existing five-minute retention limit still applies to settled jobs. Dismissal does not delete results already in the conversation or saved task records and logs. Jobs started by a sub-agent are attributed with `⟨agent-id⟩`.
